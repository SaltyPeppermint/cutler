...
class Trim:
    ...
    kind: Literal["ts", "pts", "frame"] = (
        "ts"  # whether to interpret offsets as timestamp, timecode or frame number
    )
    ...
    async def filterify(self, ctx, source):
        ...
        if self.duration is not None:
            ...
            if self.kind == "pts":
                raise RuntimeError(
                    "don't know how to compute duration of PTS-based trim with no end, FIXME"
                )
            actual_duration = await source.actual_duration.get() - self.start
        elif self.start is None:
            if self.kind == "pts":
                raise RuntimeError(
                    "don't know how to compute duration of PTS-based trim with no start, FIXME"
                )
